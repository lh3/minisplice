...
#define MSP_EVAL_BATCH_SIZE 128

/*
void msp_predict(kann_t *ann, int64_t len, const uint8_t *seq, int32_t mb_size, int32_t type)
{
	int32_t l, k;
	int64_t i;
	uint8_t x;
...
int main_predict(int argc, char *argv[])
{
	...
	while ((c = ketopt(&o, argc, argv, 1, "at:b:e:", 0)) >= 0) {
