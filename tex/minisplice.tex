...
In the ROC plot, we focused on the region with sensitivity above 50\% and false positive rate below 10\%
...
Numbers in parentheses denote rAUC, which is the Area Under the Curve restricted to the plotted region.
...
but we deemed the improvement is small and outweighed by the convenience
...
\caption{GC content of homologous introns.
Vertebrate protein sequences from Swiss-Prot are aligned to each genome with miniprot.
Given two genomes, if a protein sequence is aligned with introns at the same position on the protein, the two aligned introns are considered homologous to each other.
{\bf (a)} Comparing percent GC content between homologous human and zebrafish introns.
...
We tried to reduce the window size to 102bp but the accuracy dropped (Fig.~\ref{fig:6}a).
Increasing the window size to 302bp, on the other hand, only had a small effect (Fig.~\ref{fig:6}b).
...
%This suggests the model is learning from both splice sites and coding potentials.
...
we extracted homologous intron pairs and compared their GC content (Fig.~\ref{fig:gc}).
We saw introns reaching 60\% GC in human but their homologs in zebrafish are $<$40\% in GC (Fig.~\ref{fig:gc}a).
