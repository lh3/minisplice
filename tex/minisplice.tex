...
\vspace{0.5em}\\
\textbf{Results:}
We implemented minisplice to learn splice signals with one-dimensional convolutional neural network (1D-CNN)
and trained a model with 7,026 parameters for vetebrates and insect.
It captures conserved splice signals across phyla and reveals GC-rich introns specific to mammals and birds.
We used this model to estimate the empirical splicing probability for every {\tt GT} and {\tt AG} sites in genomes,
...
our method greatly improves the junction accuracy for noisy long reads
