...
const float *kann_apply1n_to(kann_t *a, int mini_size, float *x, int ext_flag, int ext_label)
{
	int i_out;
	i_out = kann_find(a, ext_flag, ext_label);
	if (i_out < 0) return 0;
	kann_set_batch_size(a, mini_size);
...
{
	return kann_apply1n_to(a, 1, x, ext_flag, ext_label);
}
